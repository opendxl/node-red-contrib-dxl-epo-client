/**
 * @module DxlEpoThreatEventCreate
 * @description Implementation of the `dxl-epo-threat-event-create` node
 * @private
 */
...
var EPO_CREATE_THREAT_EVENT_COMMAND = 'DxlBrokerMgmt.createEpoThreatEvent'

module.exports = function (RED) {
  /**
   * @classdesc Node which invokes the ePO 'DxlBrokerMgmt.createEpoThreatEvent'
   * command.
   ...
   * @param {(String|Object)} nodeConfig.event - Event to create.
   ...
   * @constructor
   */
  function EpoThreatEventCreate (nodeConfig) {
    ...
    if (node._client) {
      ...
      this.on('input', function (msg) {
        var event = NodeUtils.extractProperty(msg, 'event')
        if (event) {
          var params = {}
          params.event =
            (typeof event === 'object') ? JSON.stringify(event) : event
          Util.runEpoCommand(node, msg, EPO_CREATE_THREAT_EVENT_COMMAND,
            this._client.dxlClient, nodeConfig, params)
        } else {
          node.error('event was not specified')
        }
      })
  ...
  RED.nodes.registerType('dxl-epo-threat-event-create', EpoThreatEventCreate)
}
